...
internal class ComInterfaceAssemblyBuilder
{
    ...
    // The module version ID is derived from the library build. Including it in the cache key
    // prevents an interface assembly generated by older source from being loaded when the
    // package version was not incremented.
    private static readonly string _assemblyName = $"VirtualDesktop.{{0}}.{_executingAssembly.ManifestModule.ModuleVersionId:N}.generated.dll";
    private static readonly Version? _requireVersion = _executingAssembly.GetName().Version;
    ...
    {
        if (this._configuration.CompiledAssemblySaveDirectory.Exists == false) return null;

        var file = new FileInfo(Path.Combine(
            this._configuration.CompiledAssemblySaveDirectory.FullName,
            string.Format(_assemblyName, osBuild)));
    ...
    private Assembly CreateAssembly()
    {
        ...
        {
            var assemblyInfo = _executingAssembly.GetManifestResourceNames().Single(x => x.Contains("AssemblyInfo.cs"));
            var stream = _executingAssembly.GetManifestResourceStream(assemblyInfo);
            if (stream != null)
            {
        ...
        var interfaceNames = _executingAssembly
            .GetTypes()
        ...
        foreach (var name in _executingAssembly.GetManifestResourceNames())
        {
        ...
        foreach (var (interfaceName, sourceFiles) in interfaceSourceFiles)
        {
            ...
            var stream = _executingAssembly.GetManifestResourceStream(resourceName);
